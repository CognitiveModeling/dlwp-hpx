...
\begin{document}
	...
	% Maps
	\pic {annotated cuboid={width=0.7, height=4, depth=4, units=, label=input, lwidth=$c$, lheight=$n$}};
	%\pic [right=4cm of input, yshift=1.2cm] {annotated cuboid={width=0.7, height=2, depth=2, units=, label=pool, lwidth=$c$, lheight=$\frac{n}{2}$, ccolor=downscalecolor}};
	%\pic [right=4cm of input, yshift=-1.2cm] {annotated cuboid={width=0.7, height=2, depth=2, units=, label=conv, lwidth=$c$, lheight=$\frac{n}{2}$, ccolor=downscalecolor}};
	%\node[operator] at ([xshift=1.5cm]$(pool)!0.5!(conv)$) (plus) {$+$};
	%\pic [right=0.7cm of plus] {annotated cuboid={width=0.7, height=2, depth=2, units=, label=output, lwidth=$c$, lheight=$\frac{n}{2}$, ccolor=downscalecolor}};
	\pic [right=3.5cm of input] {annotated cuboid={width=0.7, height=2, depth=2, units=, label=avgpool, lwidth=$c$, lheight=$\frac{n}{2}$, ccolor=downscalecolor}};
	
	% Arrows
	%\draw[forward] (input) --++ (1, 0) -- ([xshift=1cm]input |- pool) -- (pool) node[projection] {\shortstack[l]{$2\times2$ avg pool,\\stride 2}};
	%\draw[forward] (input) --++ (1, 0) -- ([xshift=1cm]input |- conv) -- (conv) node[projection] {\shortstack[l]{$2\times2$ conv,\\stride 2}};
	%\draw[forward] (pool) -- (pool -| plus) -- (plus.north);
	%\draw[forward] (conv) -- (conv -| plus) -- (plus.south);
	%\draw[forward] (plus) -- (output);
	\draw[forward] (input) -- (avgpool) node[projection] {\shortstack[l]{$2\times2$ avg pool,\\stride 2}};
